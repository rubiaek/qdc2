...
class DiffuserSimulation:
    def __init__(self,
                 ...
                 init_off_axis=200e-6, diffuser_angle=0.5):
    ...
    def make_detection_gaussian(self, lam):
        ...
        r2 = (XX-self.init_off_axis)**2 + YY**2
    ...
    def run_SPDC_simulation(self) -> (list[float], list[Field]):
        ...
        diffuser_mask = phase_screen_diff(self.x, self.y, wl0, self.diffuser_angle)
        ...
        field_crystal.E *= np.exp(1j * diffuser_mask)
        field_det_new = propagate_free_space(field_crystal, self.dz)
        ...
        for di in range(1, N_measurements):
            ...
            field_crystal.E *= np.exp(1j * diffuser_mask*self.wl0/wl_minus)
            field_det_new = propagate_free_space(field_crystal, self.dz)
